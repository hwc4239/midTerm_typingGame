...
while life > 0:                                  # life==0 종료
    ...
    typing_speeds.append(speed)                             # 타이핑 속도 리스트

    if str(q).strip() == str(x).strip():                    # 입력(정답) 확인(공백제거)
        ########### 정답 소리 재생
        winsound.PlaySound(                                 # winsound의 PlaySound라는 클래스로 지정          
            './sound/good.wav',
            winsound.SND_FILENAME                           # SND_FILENAME을 직접 넣었음
                                                   
        )
        ############
        print(">>Pass!")
        print("남은 기회:"+str(life),end=' ')      # 남은 기회(횟수) 보여주기
        cor_cnt += 1                              # 정답 개수 카운트
...
# 수행 시간, 도전 문제 수, 정답 수, 평균 타자 속도, 평균 정확도 출력
